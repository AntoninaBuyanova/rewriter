...
export const useModelStore = create<ModelState & ModelAction>()((set, get) => ({
    ...
    setInMultipleSelectionMode: (inMultipleSelectionMode) => set(() => ({ inMultipleSelectionMode: inMultipleSelectionMode })),
    ...
    executePrompt: async (executablePrompt) => {
        ...
        try {
            const result = await ApiService.executePrompt(executablePrompt);
            
            logEvent("RESULT_FROM_GPT", {uniqueId: uniqueId, response: result.result});
    ...
    setTextFields: (textFields) => {
        ...
        forEachEditor(textFieldsToStore, e => e.id, (_, editor, textFieldId) => {
        ...
        set(() => ({ textFields: textFieldsToStore, selectedTexts: selections}))
            ...
            if (textFieldId === "mainTextField" && !(textField.state[0] as any).differences) {
                ...
                if (get().layers[0].layer.state.length === 0 || previousState.map(e => Node.string(e)).join("\n") !== newState.map(e => Node.string(e)).join("\n")) {
                    ...
                    if (updatedTextLayers) {
                        set(() => ({ layers: JSON.parse(JSON.stringify(updatedTextLayers)) }));
    ...
    setTextField: (textFieldId, newTextFieldData) => {
        ...
        set(() => ({ textFields: updatedTextFields }));
    },
    ...
    setSelectedTexts: (selectedTexts, removeTextFieldIfEmpty = false) => {
        if (selectedTexts.length === 0) {
            forEachEditor(get().textFields, e => e.id, (_, editor) => {
            ...
            const textFieldIds = [...new Set(selectedTexts.map(e => e.textFieldId))];
            forEachEditor(textFieldIds, e => e, (_, editor, textFieldId) => {
        ...
        set(() => ({ selectedTexts: selectedTexts }))
    },
     ...
     */
    animateNextChanges: () => {
        set(() => ({ animateChangesUntilTimestamp: Date.now() + 200 }))
    },
    removeLayer: (layerId) => {
        if (layerId === 0) {
    ...
    setToolOrderInToolbar: (toolsOrderInToolbar) => set(() => ({ toolsOrderInToolbar: toolsOrderInToolbar })),
    setOpenAIKey: (key) => {
        openai.apiKey = key;
      },
}))
