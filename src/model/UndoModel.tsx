...
export const useUndoModelStore = create<UndoModelState & UndoModelAction>()((set, get) => ({
    ...
    undo: () => {
        ...
        if (undoStack.length > 0) {
            ...
            if (newState) {
                ...
                set(() => ({ undoStack: undoStack, redoStack: redoStack, isDisabled: true }));
    ...
    storeUndoState: () => {
        ...
        const modelState = JSON.parse(JSON.stringify(useModelStore.getState()));
        undoStack.push(modelState);
        set(() => ({ undoStack: undoStack, redoStack: [], lastStoredStateTimestamp: Date.now() }));
    },
        ...
        if (now - lastStoredStateTimestamp > 1000) {
            if (!isDisabled) {
                ...
                if (stringifiedState !== lastStringifiedState) {
                    ...
                    set(() => ({ undoStack: [...get().undoStack, modelState], redoStack: [], lastStoredStateTimestamp: now }));
                }
            }
        } else {
            set(() => ({ lastStoredStateTimestamp: now }));
        }
    },
