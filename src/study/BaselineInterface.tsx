...
export default function BaselineInterface(props: { children?: React.ReactNode }) {
  ...
  const onMessageSend = () => {
    ...
    // Отправляем сообщение через наш ApiService
    (async () => {
      try {
        const response: MessageGPT = { content: "", role: 'assistant' };
        setGptMessages([...messages, response]);

        const result = await ApiService.executePrompt({
        ...
        useStudyStore.getState().logEvent("CHATGPT_RESPONDED", { 
          response: response.content, 
          fullHistory: gptMessages 
        });
      } catch (error) {
        console.error("Error in chat completion:", error);
        const errorMessage: MessageGPT = { 
          content: "Извините, произошла ошибка при обработке запроса.", 
          role: 'assistant' 
        };
        setGptMessages([...messages, errorMessage]);
      }
    })();
  }
        ...
        if (layer.children) {
          ...
          for (const children of layer.children) {
            ...
            if (modifications.length > 0) {
              const text = modifications.map(([_, value], idx) => `${idx > 0 ? '(' + (idx+1) + ') ' : ''}${value.trim()}`).join("\n");
              state.push({
                children: [{ text: "\n## " + children.layer.name + "\n" + text }]
              })
            }
    ...
    <>
      <div style={{ display: 'flex', flexDirection: 'row', height: '100vh', width: '100%', background: '#F2EEF0' }}>
        ...
        <div style={{ minWidth: 720, height: '100%', width: '70%', display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
          <div id="mainTextField" className={"textEditor"} style={{ 
            position: 'relative', 
            background: 'white', 
            width: 700, 
            paddingTop: 60, 
