...
export default function EditableTextField(props: {style?: React.CSSProperties, isMoveable?: boolean, textField: MoveableTextField, className?: string}) {
    ...
    const mouseDownListener = (_: React.MouseEvent) => {
    ...
    const isTextFieldEmpty = useMemo(() => {
        if (!props.textField.state || !props.textField.state[0]) return true;
        const firstNode = props.textField.state[0] as any;
        if (!firstNode.children || !firstNode.children[0]) return true;
        return firstNode.children[0].text === '';
    }, [props.textField.state]);
    ...
    return (
        ...
        style={{ 
            ...
            <TextSelection disabled={isDragging} opaque={selectedTool.isSelectionOpaque()} className={selectedTool.getMarksClassname()} style={style} onTextSelected={onTextSelected}>
                <div style={{userSelect: isDragging ? 'none' : undefined, textAlign: 'left', position: 'relative'}} className={"textEditor " + (isLoading ? "loading" : "")}>
                <Slate data-gramm={false} data-gramm_editor={false} data-enable-grammarly={false} editor={editor} initialValue={textField.state}  onChange={newValue => {
                    ...
                    (Transforms as any).removeEmptyNodes = false;
                        ...
                        <Editable 
                            ...
                            style={{userSelect: isDragging? 'none' : undefined, padding: props.isMoveable ? 5 : 0, textAlign: 'left'}} 
                            readOnly={isDragging} 
                            renderLeaf={renderLeaf} 
                            onMouseDown={mouseDownListener}
                            placeholder={props.textField.id === "mainTextField" ? "Start by writing, pasting text, or uploading a document (doc, pdf)..." : undefined}
                        />
                    </Slate>
                    
                    {/* Show EmptyStateOverlay when textfield is empty */}
                    {isTextFieldEmpty && props.textField.id === "mainTextField" && <EmptyStateOverlay textFieldId={props.textField.id} />}
                </div>
            </TextSelection>
