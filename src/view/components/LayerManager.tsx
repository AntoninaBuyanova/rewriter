...
import { LayerAddIcon, TrashIcon } from "./icons/CustomIcons";
...
export function LayerListElement(props: NodeRendererProps<HiearchicalLayer>) {
  ...
  style={{ color: isVisible ? "black" : '#AAAEB1', display: 'flex', height: 30, position: 'relative', alignItems: 'center', background: (isSelected && !props.preview) ? '#F2F2ED' : undefined, borderRadius: 2 }}
  onMouseEnter={() => {
  ...
  onMouseLeave={() => {
    ...
    <div ref={props.dragHandle} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
      <div
        style={{ ...props.style, display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 3, marginLeft: 3 }}
        onDoubleClick={(_) => {
          props.node.edit();
        }}
        >
        ...
        <span style={{ fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '220px' }}>{props.node.isEditing ? <Input node={props.node} /> : props.node.data.layer.name}</span>
      </div>
    </div>
    ...
    <CardHeader style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        <Tooltip placement="bottom" content="Add new layer" closeDelay={0}>
          <Button size="sm" variant="light" onFocus={(e) => (e.target as any)!.blur()} isIconOnly onClick={() => {
            ...
            }}><LayerAddIcon size={18} /></Button>
        </Tooltip>

        <Tooltip placement="bottom" content="Delete selected layer" closeDelay={0}>
          <Button isDisabled={selectedLayerId === 0} variant="light" onFocus={(e) => (e.target as any)!.blur()} size="sm" isIconOnly onClick={() => {
              const selectedLayer = useModelStore.getState().selectedLayerId;
              useUndoModelStore.getState().storeUndoState();
    ...
    <CardBody style={{ margin: 0, padding: 0 }}>
      <div ref={ref} style={{ display: 'flex', flexDirection: 'column', maxHeight: '100%' }}>
        <Tree data={[...layers].reverse()}
          width={'100%'}
          height={height ? Math.min(height, 30*Math.max(3, flattenedLayers.length)) : 150}
          rowHeight={30}
