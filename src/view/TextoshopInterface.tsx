...
export default function TextoshopInterface(props: { children?: React.ReactNode }) {
  ...
  return (
    <>
      <TopBar disableTonePicker={disableTonePicker} disableLayers={disableLayers} />
      
      {!disableTools && <Toolbar />}
      
      <div style={{ display: 'flex', flexDirection: 'row', height: '100vh', background: '#FFFFFF', paddingTop: isTabletOrMobile ? '60px' : '0', paddingBottom: isTabletOrMobile ? '70px' : '0' }}>
        <div id="background" style={{ 
          ...
          flexGrow: 4, 
          cursor: cursor,
          paddingLeft: isTabletOrMobile ? '0' : '90px',  // Отступ слева только на десктопе
          paddingRight: isTabletOrMobile ? '0' : '320px' // Отступ справа только на десктопе
        }}
          onMouseDown={(e) => {
        ...
        >
          ...
          {textFields.length > 0 &&
            <EditableTextField isMoveable={false} textField={textFields[0]} style={{ 
              position: 'relative', 
              background: 'white', 
              width: 700, 
              paddingTop: 60, 
          ...
          {textFields.length > 1 &&  textFields.slice(1).map((textField, index) => {
            return <EditableTextField isMoveable={true} textField={textField} key={index} style={{ display: textField.isVisible ? 'block' : 'none', boxShadow: 'none', WebkitBoxShadow: 'none', MozBoxShadow: 'none', zIndex: (sentenceHovered !== null && sentenceHovered.textfieldId === textField.id) ? 4 : 99 }} />
          })}
          ...
          {(textMergerMenuPos !== null && sentenceHoveredRects.length > 0) && <Popover offset={20} isOpen={textMergerMenuPos !== null} showArrow={true}
            ...
            shouldCloseOnInteractOutside={(_) => (Date.now() - elementDroppedTimestamp) > 200}
